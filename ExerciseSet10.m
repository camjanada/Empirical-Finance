...
clear; clc;
format bank
Kp = 90; s_A = 0.142; s_I = 0.15; rho = 0.90; r = 0.04;
Exp_returns = [0.093, 0.06];; % [max Sharpe portfolio E[r], Index E[r]]
C = [1 rho; rho 1]; 
S = diag([s_A, s_I]); 
V = S*C*S; 
returns = Exp_returns; 
...
writetable(DeltaHedgeAcct, 'DeltaHedgeAcct.csv');
...
disp(SummaryStats);
writetable(SummaryStats, 'SimulationSummary.csv');
...
writetable(AllPayoffs, 'AllSimulationPayoffs.csv');
